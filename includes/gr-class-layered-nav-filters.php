 ...
 * @extends  WC_Widget
 */
class GR_Widget_Layered_Nav_Filters extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = __( 'Shows active layered nav filters (include date filter) so users can see and deactivate them.', 'woocommerce' );
		$this->widget_id          = 'woocommerce_gr_layered_nav_filters';
		$this->widget_name        = __( 'Layered Nav & Date Filters', 'woocommerce' );
		$this->settings           = array(
			'title'  => array(
				'type'  => 'text',
	...
	public function widget( $args, $instance ) {
		...
		// Date
		$min_date = isset( $_GET['min_date'] ) ? esc_attr( $_GET['min_date'] ) : 0;
		$max_date = isset( $_GET['max_date'] ) ? esc_attr( $_GET['max_date'] ) : 0;        

		if ( 0 < count( $_chosen_attributes ) || 0 < $min_price || 0 < $max_price || 0 < $min_date || 0 < $max_date  ) {
			...
			if ( $min_date ) {
				$link = remove_query_arg( 'min_date' );
				echo '<li class="chosen"><a title="' . esc_attr__( 'Remove filter', 'woocommerce' ) . '" href="' . esc_url( $link ) . '">' . __( 'Min Date', 'woocommerce' ) . ' ' . $min_date . '</a></li>';
			}

			if ( $max_date ) {
				$link = remove_query_arg( 'max_date' );
				echo '<li class="chosen"><a title="' . esc_attr__( 'Remove filter', 'woocommerce' ) . '" href="' . esc_url( $link ) . '">' . __( 'Max Date', 'woocommerce' ) . ' ' . $max_date . '</a></li>';
			}            
